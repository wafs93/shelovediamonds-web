...
/*
  Usage:
  SANITY_PROJECT_ID=xxxx SANITY_DATASET=production SANITY_API_TOKEN=xxxx node scripts/seed-sanity-products.mjs
*/
...
if (!response.ok) {
  ...
  console.error('Sanity import failed:', response.status, text);
  process.exit(1);
}

const data = await response.json();
console.log(`Imported ${products.length} products into ${projectId}/${dataset}`);
